        ...
        for i in range(0, len(totbrick)):
            if brick_corx[i] <= ball_x <= (brick_corx[i] + 30) and brick_cory[i] <= ball_y <= (brick_cory[i] + 14):
                if brick_col[i] == 10:
                    ...
                    brick_cory.pop(i)
                    totbrick.pop(a)          
                    xball_speed = xball_speed*1.35
                    yball_speed = -yball_speed*1.35
                    score = score + 15         
                if brick_col[i] == 11:
                    brick_corx.pop(i)
                    brick_cory.pop(i)
...
def update():
    ...
    win = victory(win)
    if game == False:
        ball_x, ball_y = (vaisseau_x + 18), (vaisseau_y - 7)
...
def draw():
    ...
    if rule == True:
        pyxel.text(10, 150, "Les briques jaunes font accelerer la balle et contamine les autres briques!", 10)
