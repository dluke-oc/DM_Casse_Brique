...
def ballxbrick(ball_x, ball_y):
    ...
    for h in range(0, (len(brick_x)-1)):
        if brick_x[h] <= ball_x and ball_x <= (brick_x[h] + 30) and textop <= ball_y <= texbtom: #rebond contre brique nivsup
            brick_x.pop(h - 1)
            brick_y.pop(h - 1)
            xball_speed = xball_speed
            yball_speed = -yball_speed  
            score = score + 10
            break
    for i in range(0, (len(brick_x2)-1)):
        if brick_x2[i] <= ball_x and ball_x <= (brick_x2[i] + 30) and bextop <= ball_y <= bexbtom: #rebond contre brique  nivinf
            brick_x2.pop(i - 1)
            brick_y2.pop(i - 1)
